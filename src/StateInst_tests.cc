...
// StateInst does some mucking around with registering name-modded prototypes
// in order to deal with lifetime setting.
TEST(StateInstTests, ProtoNames) {
  std::string config = 
...
TEST(StateInstTests, BuildTimes) {
  std::string config = 
...
TEST(StateInstTests, FiniteLifetimes) {
  std::string config = 
...
TEST(StateInstTests, NoDupProtos) {
  std::string config = 
     "<prototypes>  <val>foobar</val> <val>foobar</val> <val>foobar</val> </prototypes>"
     "<build_times> <val>1</val>      <val>1</val>      <val>2</val>      </build_times>"
     "<n_build>     <val>1</val>      <val>7</val>      <val>3</val>      </n_build>"
...
cyclus::Agent* StateInstitutionConstructor(cyclus::Context* ctx) {
  return new mbmore::StateInst(ctx);
}
...
INSTANTIATE_TEST_CASE_P(StateInst, InstitutionTests,
                        Values(&StateInstitutionConstructor));
INSTANTIATE_TEST_CASE_P(StateInst, AgentTests,
                        Values(&StateInstitutionConstructor));
