...
#include "RandomSink_tests.h"

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RandomSinkTest::SetUp() {
  src_facility = new mbmore::RandomSink(tc_.get());
  trader = tc_.trader();
  InitParameters();
  SetUpRandomSink();
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RandomSinkTest::TearDown() {
  delete src_facility;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RandomSinkTest::InitParameters() {
  commod1_ = "acommod";
  commod2_ = "bcommod";
  commod3_ = "ccommod";
  capacity_ = 5;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RandomSinkTest::SetUpRandomSink() {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, InitialState) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, Clone) {
  using mbmore::RandomSink;
  RandomSink* cloned_fac = dynamic_cast<mbmore::RandomSink*>
                             (src_facility->Clone());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, DISABLED_XMLInit) {
  std::stringstream ss;
  ss << "<start>"
     << "<name>fooname</name>"
     << "<config>"
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, Requests) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, EmptyRequests) {
  using cyclus::Material;
  using cyclus::RequestPortfolio;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, InRecipe){
// Create a context
  using cyclus::RequestPortfolio;
  using cyclus::Material;
  using cyclus::Request;
  ...
  mbmore::RandomSink* snk = new mbmore::RandomSink(&ctx);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(RandomSinkTest, Print) {
  EXPECT_NO_THROW(std::string s = src_facility->str());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
cyclus::Agent* RandomSinkConstructor(cyclus::Context* ctx) {
  return new mbmore::RandomSink(ctx);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
INSTANTIATE_TEST_CASE_P(RandomSinkFac, FacilityTests, Values(&RandomSinkConstructor));
INSTANTIATE_TEST_CASE_P(RandomSinkFac, AgentTests, Values(&RandomSinkConstructor));
